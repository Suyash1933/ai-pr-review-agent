...
async def require_auth(
    ...
    # ---------- Path 1: JWT cookie (multi-user OAuth sessions) ----------
    ...
    # ---------- Path 2: X-API-Key header (backward compat) ----------
    ...
    if not provided_key:
        logger.warning(
            "require_auth | REJECTED (no JWT cookie and no X-API-Key) | path=%s | client=%s",
            request.url.path,
            request.client.host if request.client else "unknown",
        )
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Authentication required. Sign in with GitHub or provide X-API-Key.",
    ...
    logger.debug("require_auth | OK (API key) | path=%s", request.url.path)
