...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# User — GitHub OAuth user account
# ---------------------------------------------------------------------------
class User(Base):
    """A user who has signed in via GitHub OAuth."""
    ...
    github_avatar_url: Mapped[str] = mapped_column(String(512), nullable=False, default="")
    ...
    repos: Mapped[list["UserRepo"]] = relationship(
        "UserRepo", back_populates="user", lazy="selectin", cascade="all, delete-orphan",
    )
...
# ---------------------------------------------------------------------------
# UserRepo — repos a user has activated for monitoring
# ---------------------------------------------------------------------------
class UserRepo(Base):
    """A GitHub repo that a user has activated for automated PR review."""
    ...
    __table_args__ = (
        Index("ix_user_repos_user_repo", "user_id", "repo_full_name", unique=True),
        Index("ix_user_repos_repo_active", "repo_full_name", "is_active"),
    )

    def __repr__(self) -> str:
        return f"<UserRepo user={self.user_id} repo={self.repo_full_name} active={self.is_active}>"
...
class PRReviewRecord(Base):
    ...
    # The user who owns this review (GitHub OAuth). Nullable for backward
    # compatibility — legacy reviews created before multi-user have user_id=NULL.
    user_id: Mapped[str | None] = mapped_column(
        ...
        default=None,
        index=True,
        comment="FK to the user who owns this review. NULL for legacy/pre-OAuth reviews.",
    )
