...
async def receive_github_webhook(
    ...
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    # Step 2a: Try to parse repo_full_name from raw body to look up
    # per-user webhook secret BEFORE HMAC validation.
    # ------------------------------------------------------------------
    import json as _json
    user_id_for_job: str | None = None
    webhook_secret = cfg.github_webhook_secret  # default: global secret

    try:
        body_json = _json.loads(raw_body)
        repo_full_name_raw = body_json.get("repository", {}).get("full_name", "")
        if repo_full_name_raw:
            # Look up per-user webhook secret
            session_factory = get_session_factory()
            if session_factory:
                async with session_factory() as db_session:
                    result = await db_session.execute(
                        select(UserRepo).where(
                            UserRepo.repo_full_name == repo_full_name_raw,
                            UserRepo.is_active == 1,
                        )
                    )
                    user_repo = result.scalar_one_or_none()
                    if user_repo and user_repo.webhook_secret:
                        webhook_secret = user_repo.webhook_secret
                        user_id_for_job = user_repo.user_id
                        logger.debug(
                            "webhook | using per-user secret for repo=%s user=%s",
                            repo_full_name_raw, user_id_for_job,
                        )
    except Exception as e:
        logger.debug("webhook | pre-parse for user lookup failed (using global secret): %s", e)
    ...
    try:
        await enqueue_review_job(
            ...
            input_data={
                ...
                # User association (multi-user OAuth). None for legacy/global webhooks.
